...
public class GameViewManager {
    ...
    public void createNewGame(Stage menuStage, THEME chosenTHEME, PriorityQueue<Integer> scores) {
        this.scoreTable = scores;
        this.menuStage = menuStage;
        this.menuStage.hide();
        createBackground();
        createShip(chosenTHEME);
        createGameElements(chosenTHEME);
        createGameLoop();
        gameStage.show();
    }

    private void createGameElements(THEME chosenTHEME) {
        playerLife = 2;
...
//            playerLifes[i] = new ImageView(chosenTHEME.getUrlLife());
    ...
    private void moveGameElements() {
        ...
        for(int i = 0; i < brownMeteors.length; i++) {
            brownMeteors[i].setLayoutY(brownMeteors[i].getLayoutY() + 1);
    ...
    private void createShip(THEME chosenTHEME) {
        ship = new ImageView(chosenTHEME.getUrl());
    ...
    private void checkIfElementsCollide(){
        ...
        for(int i = 0; i < brownMeteors.length; i++) {
            if(METEOR_RADIUS + SHIP_RADIUS > calculateDistance(ship.getLayoutX() + 49, ship.getLayoutY() + 37, brownMeteors[i].getLayoutX() + 20, brownMeteors[i].getLayoutY() + 20)) {
                removeLife();
                setNewElementPosition(brownMeteors[i]);
            }
        }
        ...
        for(int i = 0; i < greyMeteors.length; i++) {
            if(METEOR_RADIUS + SHIP_RADIUS > calculateDistance(ship.getLayoutX() + 49, ship.getLayoutY() + 37, greyMeteors[i].getLayoutX() + 20, greyMeteors[i].getLayoutY() + 20)) {
                removeLife();
                setNewElementPosition(greyMeteors[i]);
            }
        }
