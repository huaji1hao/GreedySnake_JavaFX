...
public class GameView {
    ...
    public void drawSnake(GraphicsContext gc, Snake mySnake, int difficulty) {
        try {
            ...
            String snakeFolderPath = switch (difficulty) {
                case 1 -> "/snake/rabbit-snake/";
                case 2 -> "/snake/ghost-snake/";
    ...
    public void drawAll(GraphicsContext gc, Snake mySnake, Food[] foods, int difficulty, Meteor meteor, Gem gem, Coin coin, AIEater eater){
        ...
        if(SnakeGameController.getIsAIEater()) drawEater(gc, eater);
    }

    private void drawEater(GraphicsContext gc, AIEater eater){
        if(eater == null) return;
        double moveFrame = eater.getMoveFrame();

        for(int i = eater.getBodySize() - 1; i > 0; i--){
            Point currentPart = eater.getBodyPart(i);
            Point prevPart = eater.getBodyPart(i - 1);
            ...
            // Draw the current body part with a slight reduction in size for visual fineness
            gc.drawImage(eater.getBodyImage(), partX * SQUARE_SIZE, partY * SQUARE_SIZE, SQUARE_SIZE - 1, SQUARE_SIZE - 1);
        }

        double objectX = eater.getX();
        double objectY = eater.getY();
        switch (eater.getCurrentDirection()) {
            case KeyEventHandler.RIGHT -> objectX += moveFrame;
            case KeyEventHandler.LEFT -> objectX -= moveFrame;
            case KeyEventHandler.UP -> objectY -= moveFrame;
            case KeyEventHandler.DOWN -> objectY += moveFrame;
        }

        gc.drawImage(eater.getImage(), objectX * SQUARE_SIZE, objectY * SQUARE_SIZE, SQUARE_SIZE, SQUARE_SIZE);
    }
